...
const getCard = async (hand, x, y, angle, player) => {
  ...
  for (let i = 0; i < hand.length; i++) {
    let xDir;
    player === "d" ? (xDir = -50) : (xDir = 50);

    const cardBuffer = await fetchCardImage(hand[i].name, hand[i].suit);

    if (cardBuffer) {
      const result = await loadImage(cardBuffer);
      ctx.save();
      ctx.translate(x + xDir * i, y - 5 * i);
      ctx.rotate(angle);
...
const outcomeCanvas = (outcome, balance) => {
  ...
  drawRoundedRectangle(ctx, 74, 145, 78, 52, 35);
  ctx.fill();
...
const createCanvasImage = async (
  ...
  const playerCanvas = await getCard(playerHand, pX, pY, -0.25, "p");
  const dealerCanvas = await getCard(dealerHand, dX, dY, 2.95, "d");
  ...
  const betCanvas = await loadImage(outcomeCanvas(outcome, balance, rankId));
